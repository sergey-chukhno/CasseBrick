...
AnimatedText::AnimatedText(const sf::Font& font, const std::string& text,
    ...
    : text_(font, text, characterSize)
    , shadowText_(font, text, characterSize)
    , currentAlpha_(TEXT_PULSE_MAX_ALPHA)
    , pulseSpeed_(TEXT_PULSE_SPEED)
    , minAlpha_(TEXT_PULSE_MIN_ALPHA)
    , maxAlpha_(TEXT_PULSE_MAX_ALPHA)
...
void AnimatedText::setPulseSpeed(float speed)
{
    pulseSpeed_ = speed;
}
...
void AnimatedText::setGlowEnabled(bool enabled)
{
    glowEnabled_ = enabled;
}

void AnimatedText::setShadowEnabled(bool enabled)
{
...
{
    // Render 3 glow layers with decreasing opacity and increasing offset
    for (int i = 0; i < GLOW_LAYER_COUNT; ++i)
    {
        float layerAlpha = currentAlpha_ * (0.4f - i * 0.1f);  // Decreasing alpha
        float offsetX = static_cast<float>(i + 1) * 1.0f;  // Increasing offset
        float offsetY = static_cast<float>(i + 1) * 1.0f;

        // Create glow text with offset and alpha
        ...
        glowText.setPosition(text_.getPosition() + sf::Vector2f(offsetX, offsetY));
        window.draw(glowText);

        // Also render with negative offset for full glow
        glowText.setPosition(text_.getPosition() + sf::Vector2f(-offsetX, -offsetY));
        window.draw(glowText);
    }
}
