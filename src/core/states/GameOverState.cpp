...
GameOverState::GameOverState(Game* game, int score, int level, int bricksDestroyed)
    ...
    , titleFloatTime_(0.0f)
    , titleColorTime_(0.0f)
    , titleBaseY_(0.0f)
    , titleBasePosition_(0.0f, 0.0f)
    , statisticsColorTime_(0.0f)
    , backgroundAnimationTime_(0.0f)
{
    initializeUI();
...
void GameOverState::initializeUI()
{
    ...
    bricksDestroyedText_.setOrigin(sf::Vector2f(asteroidsBounds.size.x / 2.0f, asteroidsBounds.size.y / 2.0f));
    ...
    float startY = 450.0f;  // Adjusted from 480.0f (moved up since statistics are closer together)
...
void GameOverState::updateTitleAnimation(float deltaTime)
{
    ...
    float floatOffset = std::sin(TITLE_FLOAT_SPEED * 2.0f * static_cast<float>(M_PI) * titleFloatTime_) * TITLE_FLOAT_AMPLITUDE;
    ...
    float colorCycle = std::fmod(titleColorTime_ * TITLE_COLOR_CHANGE_SPEED, static_cast<float>(colorCount));
    ...
    switch (colorIndex1) {
        case 0: color1 = getTitleColorPink(); break;
        case 1: color1 = getTitleColorCyan(); break;
        case 2: color1 = getTitleColorPurple(); break;
        case 3: color1 = getTitleColorGreen(); break;
        case 4: color1 = getTitleColorYellow(); break;
        default: color1 = getTitleColorPink(); break;
    }
    switch (colorIndex2) {
        case 0: color2 = getTitleColorPink(); break;
        case 1: color2 = getTitleColorCyan(); break;
        case 2: color2 = getTitleColorPurple(); break;
        case 3: color2 = getTitleColorGreen(); break;
        case 4: color2 = getTitleColorYellow(); break;
        default: color2 = getTitleColorPink(); break;
    }
...
sf::Color GameOverState::lerpColor(const sf::Color& color1, const sf::Color& color2, float t)
{
    // Clamp t to [0, 1]
    t = std::max(0.0f, std::min(1.0f, t));
    
    // Linear interpolation for each color component
    unsigned char r = static_cast<unsigned char>(color1.r + (color2.r - color1.r) * t);
    unsigned char g = static_cast<unsigned char>(color1.g + (color2.g - color1.g) * t);
    unsigned char b = static_cast<unsigned char>(color1.b + (color2.b - color1.b) * t);
...
void GameOverState::updateStatisticsAnimations(float deltaTime)
{
    ...
    float scoreColorCycle = std::fmod(statisticsColorTime_ * STATISTICS_COLOR_CHANGE_SPEED, static_cast<float>(colorCount));
    ...
    float levelColorCycle = std::fmod(statisticsColorTime_ * STATISTICS_COLOR_CHANGE_SPEED + 1.0f, static_cast<float>(colorCount));
    ...
    float asteroidsColorCycle = std::fmod(statisticsColorTime_ * STATISTICS_COLOR_CHANGE_SPEED + 2.0f, static_cast<float>(colorCount));
...
void GameOverState::initializeBackground()
{
    ...
    for (int i = 0; i < PLANET_COUNT; ++i) {
        ...
        float radius = PLANET_MIN_RADIUS + static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 
        ...
        planet.shape.setOutlineColor(sf::Color(planet.color.r, planet.color.g, planet.color.b, 100));
        planet.shape.setOutlineThickness(2.0f);
        
        // Rotation properties
        planet.rotationAngle = static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 360.0f;
        planet.rotationSpeed = PLANET_MIN_ROTATION_SPEED + static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 
                              (PLANET_MAX_ROTATION_SPEED - PLANET_MIN_ROTATION_SPEED);
        
        // Orbit properties
        planet.orbitCenter = sf::Vector2f(
            windowWidth * (0.2f + static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 0.6f),
            windowHeight * (0.2f + static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 0.6f)
        );
        planet.orbitRadius = 50.0f + static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 100.0f;
        planet.orbitAngle = static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 360.0f;
        planet.orbitSpeed = PLANET_MIN_ORBIT_SPEED + static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 
                           (PLANET_MAX_ORBIT_SPEED - PLANET_MIN_ORBIT_SPEED);
        
        // Set initial position
        float orbitX = planet.orbitCenter.x + std::cos(planet.orbitAngle * static_cast<float>(M_PI) / 180.0f) * planet.orbitRadius;
        float orbitY = planet.orbitCenter.y + std::sin(planet.orbitAngle * static_cast<float>(M_PI) / 180.0f) * planet.orbitRadius;
        ...
        if (planet.isPulsarPlanet) {
            ...
            planet.glow.setFillColor(sf::Color(planet.color.r, planet.color.g, planet.color.b, 40));
            planet.glow.setPosition(sf::Vector2f(orbitX, orbitY));
            planet.pulseTime = static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 
    ...
    for (int i = 0; i < PULSAR_COUNT; ++i) {
        ...
        pulsar.baseRadius = PULSAR_MIN_RADIUS + static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 
                           (PULSAR_MAX_RADIUS - PULSAR_MIN_RADIUS);
        pulsar.glowRadius = pulsar.baseRadius * PULSAR_GLOW_MULTIPLIER;
        
        // Random position
        pulsar.position = sf::Vector2f(
            windowWidth * (0.1f + static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 0.8f),
            windowHeight * (0.1f + static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 0.8f)
        );
        ...
        pulsar.glow.setFillColor(sf::Color(pulsar.color.r, pulsar.color.g, pulsar.color.b, 50));
        pulsar.glow.setPosition(pulsar.position);
        
        // Pulse properties
        pulsar.pulseTime = static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 
                          (2.0f * static_cast<float>(M_PI));
...
void GameOverState::updateBackground(float deltaTime)
{
    ...
    for (auto& planet : planets_) {
        ...
        if (planet.hasRings) {
            for (size_t ringIndex = 0; ringIndex < planet.rings.size(); ++ringIndex) {
                ...
                float baseTilt = -10.0f + static_cast<float>(ringIndex) * 5.0f;  // Progressive tilt
                float ringRotation = planet.rotationAngle * 0.6f + baseTilt;  // Slower rotation + tilt
                // Wrap rotation to 0-360 range
                while (ringRotation >= 360.0f) ringRotation -= 360.0f;
                while (ringRotation < 0.0f) ringRotation += 360.0f;
        ...
        if (planet.isPulsarPlanet) {
            // Update pulse time
            planet.pulseTime += planet.pulseSpeed * 2.0f * static_cast<float>(M_PI) * deltaTime;
            if (planet.pulseTime >= 2.0f * static_cast<float>(M_PI)) {
                planet.pulseTime -= 2.0f * static_cast<float>(M_PI);
            }
            
            // Calculate pulse factor
            float pulseFactor = 0.5f + 0.5f * (std::sin(planet.pulseTime) + 1.0f) * 0.5f;
            float currentGlowRadius = planet.shape.getRadius() * PLANET_GLOW_MULTIPLIER * (0.7f + pulseFactor * 0.6f);
            ...
            unsigned char glowAlpha = static_cast<unsigned char>(30 + pulseFactor * 50);
            planet.glow.setFillColor(sf::Color(planet.color.r, planet.color.g, planet.color.b, glowAlpha));
        }
    }
    ...
    for (auto& pulsar : pulsars_) {
        // Update pulse time
        pulsar.pulseTime += pulsar.pulseSpeed * 2.0f * static_cast<float>(M_PI) * deltaTime;
        ...
        float pulseFactor = 0.5f + 0.5f * (std::sin(pulsar.pulseTime) + 1.0f) * 0.5f;
        float currentRadius = pulsar.baseRadius * (0.7f + pulseFactor * 0.6f);  // Varies from 0.7x to 1.3x
        float currentGlowRadius = pulsar.glowRadius * (0.6f + pulseFactor * 0.8f);  // Varies from 0.6x to 1.4x
        ...
        pulsar.glow.setFillColor(sf::Color(pulsar.color.r, pulsar.color.g, pulsar.color.b, glowAlpha));
    }
}
...
void GameOverState::createPlanetRings(Planet& planet, float planetRadius)
{
    ...
    for (int i = 0; i < static_cast<int>(PLANET_RING_COUNT); ++i) {
        ...
        if (i % 2 == 0) {
            ringColor.r = static_cast<unsigned char>(std::min(255, static_cast<int>(ringColor.r * 0.8f)));
            ringColor.g = static_cast<unsigned char>(std::min(255, static_cast<int>(ringColor.g * 0.8f)));
            ringColor.b = static_cast<unsigned char>(std::min(255, static_cast<int>(ringColor.b * 0.8f)));
        }
