...
void Cannon::render(sf::RenderWindow& window) const
{
    ...
    {
        // Stronger red blend - interpolate between cyan and red based on pulse alpha
        float redBlend = barrelRedPulseAlpha_; // 0.0 = cyan, 1.0 = red
        barrelMainWithRed.setFillColor(sf::Color(
            static_cast<unsigned char>(CANNON_PRIMARY.r * (1.0f - redBlend) + BARREL_RED.r * redBlend),
            static_cast<unsigned char>(CANNON_PRIMARY.g * (1.0f - redBlend) + BARREL_RED.g * redBlend),
            static_cast<unsigned char>(CANNON_PRIMARY.b * (1.0f - redBlend) + BARREL_RED.b * redBlend)
        ));
    }
    ...
    if (barrelRedPulseAlpha_ > 0.0f)
    {
        ...
        barrelMuzzleWithRed.setFillColor(sf::Color(
            static_cast<unsigned char>(CANNON_SECONDARY.r * (1.0f - redBlend) + BARREL_RED.r * redBlend),
            static_cast<unsigned char>(CANNON_SECONDARY.g * (1.0f - redBlend) + BARREL_RED.g * redBlend),
            static_cast<unsigned char>(CANNON_SECONDARY.b * (1.0f - redBlend) + BARREL_RED.b * redBlend)
        ));
    }
...
bool Cannon::shoot(sf::Vector2f& spawnPosition, sf::Vector2f& velocity)
{
    ...
    // Barrel attaches at top center of base, then extends forward by BARREL_LENGTH
    sf::Vector2f barrelOrigin = position_ + sf::Vector2f(0, -BASE_HEIGHT / 2.0f);
    spawnPosition = barrelOrigin + direction * (BARREL_LENGTH + BARREL_MUZZLE_LENGTH);
...
void Cannon::updateShootingEffects(float deltaTime)
{
    if (isShooting_)
    {
        ...
        // Update barrel red pulse (pulsating between 0.6 and 1.0 while shooting for more visible effect)
        // Use sine wave for smooth pulsation
        pulseTime_ += deltaTime * BARREL_RED_PULSE_SPEED;
        barrelRedPulseAlpha_ = 0.6f + 0.4f * (0.5f + 0.5f * std::sin(pulseTime_)); // Pulse between 0.6 and 1.0
    ...
    else
    {
        ...
        barrelRedPulseAlpha_ = std::max(0.0f, barrelRedPulseAlpha_ - BARREL_RED_PULSE_SPEED * deltaTime);
        lightningGlowAlpha_ = std::max(0.0f, lightningGlowAlpha_ - LIGHTNING_GLOW_SPEED * deltaTime);
        flameAnimationTime_ = 0.0f;
    }
}
...
void Cannon::renderLightningGlow(sf::RenderWindow& window) const
{
    ...
    unsigned char alpha = static_cast<unsigned char>(lightningGlowAlpha_ * 255.0f);
    
    // Render glow on base body outline
    sf::RectangleShape baseGlow(sf::Vector2f(BASE_WIDTH + 4.0f, BASE_HEIGHT + 4.0f));
    baseGlow.setOrigin(sf::Vector2f((BASE_WIDTH + 4.0f) / 2.0f, (BASE_HEIGHT + 4.0f) / 2.0f));
    baseGlow.setPosition(baseBody_.getPosition());
    baseGlow.setFillColor(sf::Color::Transparent);
    baseGlow.setOutlineColor(sf::Color(LIGHTNING_WHITE.r, LIGHTNING_WHITE.g, LIGHTNING_WHITE.b, alpha));
    baseGlow.setOutlineThickness(3.0f);
    window.draw(baseGlow);
    
    // Render glow on barrel outline
    sf::RectangleShape barrelGlow(sf::Vector2f(BARREL_LENGTH + 4.0f, BARREL_WIDTH + 4.0f));
    ...
    barrelGlow.setOutlineColor(sf::Color(LIGHTNING_WHITE.r, LIGHTNING_WHITE.g, LIGHTNING_WHITE.b, alpha));
    barrelGlow.setOutlineThickness(2.5f);
    window.draw(barrelGlow);
    
    // Render glow on side panels
    sf::RectangleShape leftPanelGlow(sf::Vector2f(BASE_PANEL_WIDTH + 3.0f, BASE_PANEL_HEIGHT + 3.0f));
    leftPanelGlow.setOrigin(sf::Vector2f((BASE_PANEL_WIDTH + 3.0f) / 2.0f, (BASE_PANEL_HEIGHT + 3.0f) / 2.0f));
    leftPanelGlow.setPosition(baseLeftPanel_.getPosition());
    leftPanelGlow.setFillColor(sf::Color::Transparent);
    leftPanelGlow.setOutlineColor(sf::Color(LIGHTNING_WHITE.r, LIGHTNING_WHITE.g, LIGHTNING_WHITE.b, alpha));
    leftPanelGlow.setOutlineThickness(2.0f);
    window.draw(leftPanelGlow);
    
    sf::RectangleShape rightPanelGlow(sf::Vector2f(BASE_PANEL_WIDTH + 3.0f, BASE_PANEL_HEIGHT + 3.0f));
    rightPanelGlow.setOrigin(sf::Vector2f((BASE_PANEL_WIDTH + 3.0f) / 2.0f, (BASE_PANEL_HEIGHT + 3.0f) / 2.0f));
    rightPanelGlow.setPosition(baseRightPanel_.getPosition());
    rightPanelGlow.setFillColor(sf::Color::Transparent);
    rightPanelGlow.setOutlineColor(sf::Color(LIGHTNING_WHITE.r, LIGHTNING_WHITE.g, LIGHTNING_WHITE.b, alpha));
...
void Cannon::renderBaseContourFlames(sf::RenderWindow& window, float intensity) const
{
...
void Cannon::renderBarrelContourFlames(sf::RenderWindow& window, float intensity, float /* cosAngle */, float /* sinAngle */) const
{
...
void Cannon::renderShapeContourFlames(sf::RenderWindow& window, const sf::RectangleShape& shape, float intensity) const
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        worldCorners[i] = sf::Vector2f(
            x * cosRot - y * sinRot + position.x,
    ...
    {
        int next = (i + 1) % 4;
        renderEdgeFlames(window, worldCorners[i], worldCorners[next], intensity, false);
    }
}

void Cannon::renderEdgeFlames(sf::RenderWindow& window, const sf::Vector2f& start, const sf::Vector2f& end, float intensity, bool /* isBarrel */) const
{
    ...
    {
        float t = (numFlames > 1) ? (static_cast<float>(i) / static_cast<float>(numFlames - 1)) : 0.0f;
        ...
        float sizeVariation = 0.3f * std::sin(flameTime * 6.0f) + 0.2f * std::sin(flameTime * 9.0f);
        ...
        {
            float layerOffset = static_cast<float>(layer) / static_cast<float>(CONTOUR_FLAME_POINTS);
            ...
            if (layerOffset < 0.5f)
            {
                ...
                flameColor = sf::Color(
                    static_cast<unsigned char>(FLAME_MID.r * (1.0f - t) + FLAME_OUTER.r * t),
                    static_cast<unsigned char>(FLAME_MID.g * (1.0f - t) + FLAME_OUTER.g * t),
                    static_cast<unsigned char>(FLAME_MID.b * (1.0f - t) + FLAME_OUTER.b * t)
                );
            }
            else
            {
                ...
                flameColor.a = static_cast<unsigned char>(255 * (1.0f - t * 0.6f) * intensity);
            }
            
            // Apply intensity-based alpha
            flameColor.a = static_cast<unsigned char>(flameColor.a * intensity * (1.0f - layerOffset * 0.4f));
            
            // Calculate layer-specific position offset
            float layerFlicker = 1.0f * std::sin(layerTime * 5.0f) * (1.0f - layerOffset);
