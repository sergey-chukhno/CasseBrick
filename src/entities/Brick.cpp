...
Brick::Brick(const sf::Vector2f& position, const sf::Vector2f& size, int maxHealth, const sf::Color& baseColor, float distanceFactor)
    : position_(position)
    , size_(size)
    , health_(maxHealth)
    , maxHealth_(maxHealth)
    , isDestroyed_(false)
    , distanceFactor_(distanceFactor)  // Store distance factor for gradient (0.0 = edge, 1.0 = center)
    , shape_(size)
    , baseColor_(baseColor)
    , currentColor_(baseColor)
    , healthText_(FontManager::getDefaultFont(), "", HEALTH_TEXT_SIZE)
    , rotationAngle_(static_cast<float>(std::rand() % 360))  // Random starting rotation
    , rotationSpeed_(ROTATION_SPEED_MIN + static_cast<float>(std::rand() % static_cast<int>(ROTATION_SPEED_MAX - ROTATION_SPEED_MIN)))
    , pulseAlpha_(PULSE_ALPHA_MAX)
    , glowIntensity_(GLOW_INTENSITY_MAX)
    , animationTime_(static_cast<float>(std::rand() % 100) / 100.0f)  // Random starting animation time
{
...
sf::FloatRect Brick::getBounds() const
{
    if (isDestroyed_) {
        ...
        return sf::FloatRect(sf::Vector2f(0, 0), sf::Vector2f(0, 0)); // Empty bounds
    }
...
void Brick::updateHealthText()
{
    ...
    healthText_.setOrigin(sf::Vector2f(textBounds.size.x / 2.0f, textBounds.size.y / 2.0f));
    healthText_.setPosition(position_);
}

void Brick::renderGlow(sf::RenderWindow& window, const sf::RectangleShape& shape, const sf::Color& baseColor) const
{
    // Render multiple glow layers with decreasing opacity (enhanced for strong asteroid effect)
    for (int i = 0; i < GLOW_LAYERS; ++i) {
        ...
        float baseAlpha = static_cast<float>(GLOW_ALPHA_BASE - (i * GLOW_ALPHA_DECREMENT));
        ...
        sf::Vector2f scaledSize = sf::Vector2f(originalSize.x * scale, originalSize.y * scale);
        ...
        // Apply glow intensity to color (brighten more for stronger asteroid effect)
        // Use a wider range for more visible color variation
        float colorIntensity = 0.7f + (glowIntensity_ * 0.3f);  // Range: 0.7-1.0 (brighter)
        unsigned char r = static_cast<unsigned char>(std::min(255.0f, baseColor.r * colorIntensity));
        unsigned char g = static_cast<unsigned char>(std::min(255.0f, baseColor.g * colorIntensity));
        unsigned char b = static_cast<unsigned char>(std::min(255.0f, baseColor.b * colorIntensity));
        
        // Add brightness boost to glow layers for more visibility (makes glow stand out)
        ...
        glowShape.setFillColor(sf::Color(r, g, b, static_cast<unsigned char>(alpha)));
        glowShape.setOutlineColor(sf::Color(r, g, b, static_cast<unsigned char>(alpha)));
        glowShape.setOutlineThickness(OUTLINE_THICKNESS * scale * 1.2f);  // Thicker outline for glow

        window.draw(glowShape);
    }
}

sf::Color Brick::adjustBrightness(const sf::Color& color, float distanceFactor)
{
    // Brightness adjustment: darker = center, lighter = edges (VERY pronounced gradient)
    // distanceFactor = 1.0 (center) -> darker
    // distanceFactor = 0.0 (edge) -> lighter
    ...
    float brightness = minBrightness + (1.0f - distanceFactor) * (maxBrightness - minBrightness);
    ...
    unsigned char r = static_cast<unsigned char>(std::max(0.0f, std::min(255.0f, rFloat)));
    unsigned char g = static_cast<unsigned char>(std::max(0.0f, std::min(255.0f, gFloat)));
    unsigned char b = static_cast<unsigned char>(std::max(0.0f, std::min(255.0f, bFloat)));
...
void Brick::updateAnimations(float deltaTime)
{
    ...
    // Update pulse animation using sine wave for smooth glow (asteroid-like pulsing)
    // Use sine wave: sin(2π * frequency * time) maps to [-1, 1]
    // Map to [PULSE_ALPHA_MIN, PULSE_ALPHA_MAX]
    float sineValue = std::sin(2.0f * static_cast<float>(M_PI) * PULSE_SPEED * animationTime_);
    pulseAlpha_ = PULSE_ALPHA_MIN + (sineValue + 1.0f) / 2.0f * (PULSE_ALPHA_MAX - PULSE_ALPHA_MIN);

    // Update glow intensity (synchronized with pulse)
    glowIntensity_ = PULSE_ALPHA_MIN + (pulseAlpha_ - PULSE_ALPHA_MIN) * 
                     ((GLOW_INTENSITY_MAX - GLOW_INTENSITY_MIN) / (PULSE_ALPHA_MAX - PULSE_ALPHA_MIN));
}
